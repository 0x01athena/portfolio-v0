      ...
      >
        <div className="flex justify-between items-center p-4 mx-auto max-w-[1440px] h-[80px]">
          <a href="#" className="font-bold text-xl tracking-wider">
            <span className="text-teal-600">Daichi</span>{" "}
            <span className="text-white">Saito</span>
          </a>
          <div className="hidden md:flex md:justify-between items-center md:space-x-10 font-semibold">
            {navItems.map((item, index) => (
              <a
                href={item.to}
                key={index}
                className={`group flex items-center transform transition hover:text-teal-500 ${activeSection === item.to.substring(1) ? "text-teal-500" : ""
                  }`}
